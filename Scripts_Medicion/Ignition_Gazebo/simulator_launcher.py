    ...
    def __init__(self, mode):
        ...
        self.sim_cmd_map = {
            "sin_gpu_gui": [
                ...
                "nav2:=true slam:=false localization:=true rviz:=true world:=maze map:=my_map_maze.yaml gui:=true"
            ],
            "con_gpu_gui": [
                "bash", "-c",
                "__NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia ros2 launch turtlebot4_ignition_bringup "
                "turtlebot4_ignition.launch.py nav2:=true slam:=false localization:=true rviz:=true world:=maze "
                "map:=my_map_maze.yaml gui:=true"
            ],
            "sin_gpu_sin_gui": [
                "bash", "-c",
                "ros2 launch turtlebot4_ignition_bringup turtlebot4_ignition.launch.py "
                "nav2:=true slam:=false localization:=true rviz:=true world:=maze map:=my_map_maze.yaml gui:=false"
            ],
            "con_gpu_sin_gui": [
                "bash", "-c",
                "__NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia ros2 launch turtlebot4_ignition_bringup "
                "turtlebot4_ignition.launch.py nav2:=true slam:=false localization:=true rviz:=true world:=maze "
                "map:=my_map_maze.yaml gui:=false"
            ]
        }
